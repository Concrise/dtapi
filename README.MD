...
- `current_input_file`：全局生效的上下文拆分上传策略；默认开启，达到阈值时会将完整上下文合并上传为 `conversation_context.txt` 上下文文件；Pro/专家模式会强制跳过该文件路径。
- `current_input_file` 关闭时，请求会直接透传，不上传拆分上下文文件；Flash 保留文件上传长上下文能力。
