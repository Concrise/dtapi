...
- `current_input_file`: the global context split/upload mode; it is enabled by default and uploads the full context as a `conversation_context.txt` context file once the character threshold is reached. Pro/expert mode always skips this file path.
- With `current_input_file` disabled, requests pass through directly without uploading any split context file. Flash keeps file-upload long-context support.
