...
- current input file 触发时生成的 `conversation_context.txt` 上下文文件
...
- `current_input_file` 默认开启；但 Pro/专家模式会强制跳过该文件路径。开启后，它在统一 completion runtime 入口全局生效，用于把“完整上下文”合并进 `conversation_context.txt` 上下文文件。当最新 user turn 的纯文本长度达到 `current_input_file.min_chars`（默认 `0`）时，runtime 会上传一个文件名为 `conversation_context.txt` 的上下文文件。文件内容会先经过各协议入口的标准化，再序列化成按轮次编号的 `conversation_context.txt` 风格 transcript，带有 `# conversation_context.txt` 标题和 `=== N. ROLE ===` 分段；如果当前请求声明了可用工具，还会把工具名称、描述和参数 schema 单独上传成 `tool_definitions.txt`，带有 `# tool_definitions.txt` 标题。live prompt 中则会给出一个 continuation 语气的 user 消息，引导模型从 `conversation_context.txt` 的最新状态继续推进，并在有工具文件时明确可用工具 schema 位于 `tool_definitions.txt`；system prompt 也会在统一 DSML 工具格式约束前说明 `tool_definitions.txt` 是可调用工具和 schema 的权威来源，同时保留本轮工具选择策略，避免把任务拉回起点。
...
当前输入转文件启用并触发时，上传的历史文件真实文件名是 `conversation_context.txt`，文件内容是完整 `messages` 上下文；它会使用 OpenAI-compatible 的消息/transcript 序列化规则和 DeepSeek 角色标记，再按轮次编号成 `conversation_context.txt` 风格的 transcript（不再注入文件边界标签）：

```text
[uploaded filename]: conversation_context.txt
...
开启后，请求的 live prompt 不再直接内联完整上下文，也不再内联大段工具 schema；它保留一个 user role 的短提示，提示模型基于已提供上下文直接回答最新请求，并在有工具时引用 `tool_definitions.txt`。上传后的 `conversation_context.txt` file_id 会排在 `ref_file_ids` 最前；如果存在 `tool_definitions.txt`，它的 file_id 紧随其后；客户端已有的其他 file_id 保持在后面。上下文 token 统计会包含上传的历史文件、工具文件和 live prompt。自动生成的 current-input 文件引用会被记录为 runtime 状态；如果托管账号模式切号 fresh retry，runtime 会重新上传这些自动文件，而不是把上一账号的 file_id 交给新账号。
...
- 大部分结构化语义被压进 `prompt`
- 文件保持文件
- 需要时把完整上下文拆进 `conversation_context.txt` 上下文文件，并按轮次编号成 transcript
...
- current input file 触发条件、上传格式、`conversation_context.txt` transcript 结构变更
