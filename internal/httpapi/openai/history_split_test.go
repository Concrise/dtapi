...
func TestBuildOpenAICurrentInputContextTranscriptUsesNumberedHistorySections(t *testing.T) {
	...
	if !strings.Contains(transcript, "# conversation_context.txt") {
...
func TestApplyCurrentInputFileSkipsExpertModel(t *testing.T) {
	ds := &inlineUploadDSStub{}
	h := &openAITestSurface{
		Store: mockOpenAIConfig{
			currentInputEnabled: true,
	...
	if out.CurrentInputFileApplied || len(out.RefFileIDs) != 0 {
		t.Fatalf("expected direct prompt for expert model, got current_input=%v refs=%#v", out.CurrentInputFileApplied, out.RefFileIDs)
	}
	if !strings.Contains(out.FinalPrompt, "first user turn") || !strings.Contains(out.FinalPrompt, "latest user turn") {
		t.Fatalf("expected expert model to keep direct message context, got %s", out.FinalPrompt)
	}
}
...
func TestApplyCurrentInputFileUploadsFirstTurnWithNumberedHistoryTranscript(t *testing.T) {
	...
	if upload.Filename != "conversation_context.txt" {
	...
	if !strings.Contains(out.FinalPrompt, "Continue from the latest state in the attached conversation_context.txt context.") {
	...
	if !strings.Contains(out.PromptTokenText, "# conversation_context.txt") || !strings.Contains(out.PromptTokenText, "=== 1. USER ===") {
...
func TestApplyCurrentInputFilePreservesFullContextPromptForTokenCounting(t *testing.T) {
	...
	if !strings.Contains(out.PromptTokenText, "# conversation_context.txt") || !strings.Contains(out.PromptTokenText, "=== 1. SYSTEM ===") {
		t.Fatalf("expected prompt token text to include numbered history transcript, got %q", out.PromptTokenText)
	}
	if !strings.Contains(out.PromptTokenText, "Continue from the latest state in the attached conversation_context.txt context.") {
...
func TestApplyCurrentInputFileUploadsFullContextFile(t *testing.T) {
	...
	upload := ds.uploadCalls[0]
	if upload.Filename != "conversation_context.txt" {
		t.Fatalf("expected conversation_context.txt upload, got %q", upload.Filename)
	}
	if upload.ModelType != "vision" {
		t.Fatalf("expected vision model type for vision request, got %q", upload.ModelType)
	}
	uploadedText := string(upload.Data)
	for _, want := range []string{"# conversation_context.txt", "=== 1. SYSTEM ===", "=== 2. USER ===", "=== 3. ASSISTANT ===", "=== 4. TOOL ===", "=== 5. USER ===", "system instructions", "first user turn", "hidden reasoning", "tool result", "latest user turn", promptcompat.ThinkingInjectionMarker} {
		if !strings.Contains(uploadedText, want) {
...
func TestApplyCurrentInputFileUploadsToolsContextSeparately(t *testing.T) {
	...
	if ds.uploadCalls[0].Filename != "conversation_context.txt" {
		t.Fatalf("expected first upload to be conversation_context.txt, got %q", ds.uploadCalls[0].Filename)
	}
	...
	if !strings.Contains(out.FinalPrompt, "Continue from the latest state in the attached conversation_context.txt context.") || !strings.Contains(out.FinalPrompt, "tool_definitions.txt") {
	...
	if !strings.Contains(out.PromptTokenText, "# conversation_context.txt") || !strings.Contains(out.PromptTokenText, "# tool_definitions.txt") || !strings.Contains(out.PromptTokenText, "Description: search docs") {
...
func TestApplyCurrentInputFileCarriesHistoryText(t *testing.T) {
	...
	if !strings.Contains(out.HistoryText, "# conversation_context.txt") || !strings.Contains(out.HistoryText, "=== 1. SYSTEM ===") {
...
func TestChatCompletionsCurrentInputFileUploadsContextAndKeepsNeutralPrompt(t *testing.T) {
	...
	if !strings.Contains(historyText, "# conversation_context.txt") || !strings.Contains(historyText, "=== 1. SYSTEM ===") {
	...
	if !strings.Contains(promptText, "Continue from the latest state in the attached conversation_context.txt context.") {
...
func TestCurrentInputFileWorksAcrossAutoDeleteModes(t *testing.T) {
	for _, mode := range []string{"none", "single", "all"} {
		t.Run(mode, func(t *testing.T) {
			...
			promptText, _ := ds.completionReq["prompt"].(string)
			if !strings.Contains(promptText, "Continue from the latest state in the attached conversation_context.txt context.") || strings.Contains(promptText, "first user turn") || strings.Contains(promptText, "latest user turn") {
